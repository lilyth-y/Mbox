    ...
    <div className="space-y-6">
      <div className="mbox-card">
        ...
        <p className="text-sm text-mbox-muted leading-relaxed">
          3D 큐브의 인물·배경 분리(패럴랙스)는 <strong className="text-mbox-gold">누끼</strong>처럼 배경이
        ...
        <div className="mt-4 flex flex-col gap-2">
          <button
            ...
            className="w-full rounded-xl border border-mbox-gold/45 bg-mbox-gold/15 py-3 text-sm font-semibold text-mbox-gold transition hover:bg-mbox-gold/25 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isProcessing ? (
          ...
          <button
            ...
            className="w-full rounded-xl border border-mbox-gold/35 bg-mbox-gold/10 py-3 text-sm font-semibold text-mbox-gold transition hover:bg-mbox-gold/20 disabled:cursor-not-allowed disabled:opacity-50"
          >
            선택 이미지에만 배경 제거
          </button>
        </div>
        {galleryCount > 0 && pendingCutoutCount === 0 ? (
          <p className="mt-2 text-[11px] text-mbox-gold/90">모든 이미지에 배경 제거가 적용되었습니다.</p>
        ) : null}
      </div>
      ...
      <div className="mbox-card">
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          {BACKGROUND_TEMPLATES.map((template) => {
            ...
            return (
              ...
              >
                <p className="text-sm font-semibold text-mbox-text">{template.label}</p>
                <p className="mt-1 text-[11px] leading-relaxed text-mbox-subtle">{template.description}</p>
              </button>
            );
          })}
        </div>
        ...
        <button
          ...
          disabled={!selectedImageLabel || isProcessing}
          onClick={onApply}
          className="mt-4 w-full gold-btn rounded-xl py-3 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50"
        >
          {isProcessing ? (
            <span className="inline-flex items-center justify-center gap-2">
              <Loader2 className="animate-spin" size={16} />
        ...
        <p className="mt-3 text-[11px] leading-relaxed text-mbox-subtle">
          {selectedImageLabel
